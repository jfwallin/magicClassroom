...
class SeInfoOrSerial(ctypes.Union):
    ML_TAIL_DATA_SEINFO_MAXSIZE = 64
    ML_TAIL_DATA_SERIAL_MINSIZE = 12
    ML_TAIL_DATA_SERIAL_MAXSIZE = 32

    _pack_ = 1
    _fields_ = [
        ("seinfo", ctypes.c_char * ML_TAIL_DATA_SEINFO_MAXSIZE),
        ("serial", ctypes.c_char * ML_TAIL_DATA_SERIAL_MAXSIZE),
    ]
...
class ML_TAIL_DATA(ctypes.Structure):
    ...
    def __init__(self,
                 ...
                 sentinel_version=0,
                 id=0,
                 union = SeInfoOrSerial(b"", b""),
                 taildata_ver=ML_TAIL_DATA_LATEST_VERSION,
                 tail_data_magic=ML_TAIL_DATA_MAGIC_INT):
        super(ML_TAIL_DATA, self).__init__(devcert_len, profile_tag, is_debuggable, sentinel_type, sentinel_version, id, union, taildata_ver, tail_data_magic)
