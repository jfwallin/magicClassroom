...
enum {
  /*! Internal data blob size for #MLDataArrayDiff. */
...
/*!
  ...
  \retval MLResult_InvalidParam The out_diff parameter was not valid (null).
  \retval MLResult_Ok The operation successed and out_diff contains an initialized diff.
  \retval MLResult_UnspecifiedFailure Failed to initialize the diff.

  \priv None
*/
ML_API MLResult ML_CALL MLDataArrayInitDiff(MLDataArrayDiff *out_diff);
...
/*!
  ...
  \param[in,out] inout_diff Optional in/out parameter to prevent data from being retrieved
                 if it hasn't changed since a previous change.

  \retval MLResult_Locked If the array is already locked (and thus could not obtain the lock).
  \retval MLResult_Ok If new data is available, and the array has successfully been locked.
  \retval MLResult_Pending The array contains no new data (array will not be locked after returning this result).
  \retval MLResult_UnspecifiedFailure The operation failed for an unspecified reason.

  \priv None
*/
ML_API MLResult ML_CALL MLDataArrayTryLock(MLDataArrayHandle data_array, MLDataArray *out_data, MLDataArrayDiff *inout_diff);
...
/*!
  ...
  \retval MLResult_Ok The data array was successfully unlocked.
  \retval MLResult_UnspecifiedFailure The data array failed to be unlocked.

  \priv None
*/
ML_API MLResult ML_CALL MLDataArrayUnlock(MLDataArrayHandle data_array);
